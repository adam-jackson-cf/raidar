...
| 2026-06-12 | benchmark-view deprecated and removed; comparison value ported to review-surface | User decision: review-surface realises the intended Workshop-led UX direction. Ported: revision movement deltas, scenario contract diff with comparability warnings, failure-pattern rollups, score-vs-duration scatter. Deliberately dropped: decision-score heuristic, signal coverage matrix, improvement playbook, scenario portfolio census (superseded by deterministic findings or out of review scope). | `benchmark-view/` and its make targets removed; `make review-surface-*` is the review interface. Earlier tracker references to benchmark-view are historical record. |
| 2026-06-12 | review-surface comparison UX rebuilt as Scenario Board + Experiment Review per the review-surface specs | Design review against the review-surface spec set found the Experiments page was the "metric dump" the design principles prohibit: composite-score-led table, silent best-of comparison, raw metric ids, no verdicts/confidence/evidence. | New derived presentation model (`review-surface/scripts/derive-review.mjs` → `data/review.json`, `/api/review`): representative-experiment selection, five canonical dimensions with hard overrides, pinned benchmarks (`review.config.json`, no current-best fallback), confidence bands gating verdict language, evidence proof blocks with run deep-links, run consistency with outlier reasons, and at most three hypothesis-shaped recommendations. Visual scenarios get a reference/current/benchmark/diff strip with region cards served from `/artifacts/`; a synthetic `homepage-hero-replication` fixture (two AgentSpecs, generated PNGs) validates the visual evidence model. Composite-score table removed. |
