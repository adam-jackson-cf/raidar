"""Tests for trace log parsing across agents."""
...
def test_parse_cursor_trace_jsonl(tmp_path):
    ...
    events = parse_cursor_trace(tmp_path)
...
def test_parse_copilot_trace_gate_and_tool(tmp_path):
    ...
    events = parse_trace("copilot", tmp_path)
...
def test_parse_trace_unknown_agent(tmp_path):
    """Unknown agent should return an empty trace event list."""
    events = parse_trace("claude-code", tmp_path)
