// Adapted from Raindrop Workshop (MIT) — app/src/components/JsonView.tsx
...
const JsonNode: React.FC<NodeProps> = ({ keyName, value, depth, maxExpand, isLast }) => {
  ...
  if (n === 0) {
    return (
      <div style={{ ...mono, paddingLeft: depth * INDENT }}>
        {keyEl}
        <span style={{ color: dc.brace, fontFamily: MONO }}>
          {br[0]}
          {br[1]}
        </span>
        <span style={{ color: dc.comma, fontFamily: MONO }}>{trail}</span>
  ...
  if (!open) {
    return (
      <div
        style={{ ...mono, paddingLeft: depth * INDENT, cursor: 'pointer' }}
        onClick={(e) => {
          e.stopPropagation();
          setOpen(true);
        }}
    ...
    <div>
      ...
      <div style={{ position: 'relative' }}>
        <div
          ...
          onMouseEnter={(e) => {
            e.currentTarget.style.borderLeftColor = dc.guideHover;
          }}
          onMouseLeave={(e) => {
            e.currentTarget.style.borderLeftColor = dc.guide;
          }}
        />
        {entries.map(([k, v], i) => (
          <JsonNode
